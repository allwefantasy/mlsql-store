...
impl PluginsStore {
    ...
    pub fn query<T>(
        ...
        sql: &str,
        params: T,
    ) -> Result<Vec<PluginsStore>, my::error::Error>
    where
        T: Into<Params>,
    {
        let db_ref = db.value();
        let query = async move |tx_c: Sender<Result<Vec<PluginsStore>, my::error::Error>>| {
            let query_res = db_ref.prep_exec(sql, params).unwrap();

            let res = query_res
                .map(|x| x.unwrap())
                .map(|row| {
                    let (id, name, location, official, enable) =
                        my::from_row::<(i32, String, String, i32, i32)>(row);
                    PluginsStore {
                        id,
                        name,
                        location,
